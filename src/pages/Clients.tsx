...
import { collection, query, orderBy, getDocs, doc, deleteDoc, updateDoc, where, writeBatch } from 'firebase/firestore';
...
export const Clients: React.FC = () => {
  ...
  const handleToggleVisibility = async (client: Client) => {
    try {
      ...
      await updateDoc(clientRef, {
        isIconsVisible: !client.isIconsVisible
      });

      // Обновляем категории клиента
      const [projectsQuery, clientsQuery] = [
        query(
          collection(db, 'categories'),
      ...
      [...projectsSnapshot.docs, ...clientsSnapshot.docs].forEach(doc => {
        batch.update(doc.ref, { isVisible: !client.isIconsVisible });
      });
      ...
      setClients(prevClients => 
        prevClients.map(c => 
          c.id === client.id 
            ? { ...c, isIconsVisible: !c.isIconsVisible }
            : c
        )
      );
    } catch (error) {
